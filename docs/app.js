...
function renderResults(items, showPaths, oneBased) {
  if (items.length === 0) return `<p class="muted">No words found.</p>`;
...
async function loadBuiltInWords(minLen) {
  const res = await fetch("./words.txt", { cache: "no-store" });
  if (!res.ok) throw new Error("Could not load words.txt (make sure it exists in docs/ and is pushed).");
  ...
  if (words.length === 0) throw new Error("Built-in words.txt loaded but produced 0 usable words.");
  return words;
}
...
document.getElementById("solveBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const words = await loadBuiltInWords(minLen);
    ...
    const found = solveWithPaths(grid, trie, minLen);
